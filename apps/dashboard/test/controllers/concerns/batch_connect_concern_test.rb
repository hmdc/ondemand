...
class BatchConnectConcernTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @target.user_configuration = stub(:apps_menu => nil)
    @target.nav_bar = []
  end

  test "bc_custom_apps_group should return nil when no apps_menu and nav_bar defined" do
    assert_nil @target.bc_custom_apps_group
  end

  test "bc_custom_apps_group should return a navigation menu based on apps_menu property when defined" do
    @target.user_configuration = stub(:apps_menu => {title: "Custom Apps Menu", links: []})
  ...
  test "bc_custom_apps_group should return a navigation menu based on @nav_bar when apps_menu is not defined and @nav_bar is" do
