...
class PinnedAppsHelperTest < ActionView::TestCase
  ...
  test 'redirect_to_referer? should return false when pinned_apps_redirect is false' do
    ...
    link_data = { data: { method: 'post' } }
    link = OodAppLink.new(link_data)

    assert_equal false, redirect_to_referer?(link)
  end

  test 'redirect_to_referer? should return false when pinned_apps_redirect is true and link.data method is not post' do
  ...
  test 'redirect_to_referer? should return true when pinned_apps_redirect is true and link.data method is post' do
    @user_configuration.stubs(:pinned_apps_redirect).returns(true)
