...
class PinnedAppTest < ActiveSupport::TestCase
  def setup
    PinnedApp.instance_variable_set('@pinned_apps', nil)
  ...
  test 'pinned apps returns at least empty' do
    ...
    assert_equal [], PinnedApp.pinned_apps([], [])
  end
  ...
  test 'pinned apps with specific dev apps' do
    ...
    pinned_apps = PinnedApp.pinned_apps(tokens, all_apps)
  ...
  test 'pinned apps with wildcarded sys apps' do
    ...
    pinned_apps = PinnedApp.pinned_apps(['sys/*'], all_apps)
  ...
  test 'nils return nothing' do
    ...
    pinned_apps = PinnedApp.pinned_apps(nil, nil)
    assert_equal [].to_set, pinned_apps.map(&:token).to_set
  end
end
