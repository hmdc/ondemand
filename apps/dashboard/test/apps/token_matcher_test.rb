...
class TokenMatcherTest < ActiveSupport::TestCase
  ...
  test "token should be return token configuration as hash" do
    target = TokenMatcher.new("sys/app")
    assert_equal({token: "sys/app"} , target.token)
  ...
  test "type, category, subcategory, token, title, sub_title, and icon_uri should not trigger metadata match" do
